...
```bash
python3 camera_calibration/cameraCalibration.py
```

This uses a chessboard pattern (9×6 inner corners) detected in images from `camera_calibration/calibration_data/images/`, and saves the result to `calibration_data/calibration_parameters.npz`.
A calibration_parameters.npz is provided. To recalibrate, put images gathered from Step 1 in `camera_calibration/calibration_data/images/`
...
This subscribes to the raw image topic, applies undistortion using the `calibration_parameters.npz` file from Step 2, auto-crops to the valid region, and republishes both the corrected image and a matching `CameraInfo` message. If recalibrated in step 2, replace the `calibration_parameters.npz` file in `ros2_ws/src/img_tools/calibration`.

| Parameter | Default | Description |
|---|---|---|
| `input_topic` | `/zed/zed_node/rgb/color/raw/image` | Raw image source |
| `output_image_topic` | `/zed/zed_node/rgb/color/undistorted_cropped/image` | Undistorted output |
| `output_camera_info_topic` | `/zed/zed_node/rgb/color/undistorted_cropped/camera_info` | Matching CameraInfo |

Crop is based on roi from the `cv.getOptimalNewCameraMatrix()` function. To manually crop run the node with `--ros-args`:
```bash
ros2 run img_tools undistort_image \
  --ros-args \
  -p crop_x:=200 \
...
## Recording, Synchronizing and Playing Back Datasets
...
To synchronize image-pointcloud pairs using a threshold (ensuring time synchronization in addition to PTP):
```bash
python3 sample_prune.py sample_rosbag/sample_rosbag.db3 sample_rosbag_pruned
```
...
| `camera_calibration/cameraCalibration.py` | OpenCV chessboard calibration. Detects a 9×6 corner pattern in `camera_calibration/calibration_data/images/`, computes intrinsics, and saves to `camera_calibration/calibration_parameters.npz`. |
